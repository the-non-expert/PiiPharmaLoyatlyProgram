...
<main class="min-h-screen bg-[#F4F6F8] flex flex-col font-[Montserrat]">
  <div class="max-w-lg mx-auto w-full flex flex-col flex-1">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4">
      ...
      {#if data.claims.length === 0 && data.inProgress.length === 0}
        <div class="flex flex-col items-center justify-center pt-20 gap-4">
          <div class="w-[90px] h-[90px] rounded-xl flex items-center justify-center border-2 border-dashed border-[#EAEAEA] bg-[#F4F6F8]">
          ...
          <a href="/app" class="mt-2 bg-[#2372B9] text-white rounded-lg px-8 py-3 text-[14px] font-bold">
      ...
      {:else}
        <div class="flex flex-col gap-[10px]">
          ...
          <!-- In-progress cards (shown first — actionable) -->
          {#if data.inProgress.length > 0}
            <div class="text-[11px] font-bold text-[#686868] uppercase tracking-wider px-1 mb-0.5">In Progress</div>
            {#each data.inProgress as item}
              {@const remaining = item.coupons_required - item.scanned}
              <div class="bg-white rounded-xl border border-[#F59E0B] px-4 py-4" style="box-shadow:0 1px 6px rgba(245,158,11,0.10);">
                <!-- Product name + progress label -->
                <div class="flex items-start justify-between mb-2 gap-2">
                  <div class="text-[15px] font-bold text-[#474545] leading-snug">{item.name}</div>
                  <span class="shrink-0 text-[11px] font-bold px-2 py-0.5 rounded-full" style="background:#fef3cd;color:#92640a;">
                    {item.scanned}/{item.coupons_required}
                  </span>
                </div>

                <!-- Progress dots -->
                <div class="flex gap-2 mb-3">
                  {#each { length: item.coupons_required } as _, i}
                    <div
                      class="w-3 h-3 rounded-full transition-colors"
                      style="background:{i < item.scanned ? '#F59E0B' : '#EAEAEA'}"
                    ></div>
                  {/each}
                </div>

                <!-- Motivational copy -->
                <p class="text-[13px] text-[#686868] mb-3 leading-relaxed">
                  Scan {remaining} more coupon{remaining === 1 ? '' : 's'} to unlock
                  <span class="font-bold text-[#474545]">₹{item.cashback_amount} cashback</span>
                </p>

                <!-- CTA -->
                <a
                  href="/app"
                  class="flex items-center justify-center gap-1.5 rounded-lg py-2.5 text-[13px] font-bold text-white"
          ...
          {#if data.claims.length > 0}
            {#if data.inProgress.length > 0}
              <div class="text-[11px] font-bold text-[#686868] uppercase tracking-wider px-1 mt-2 mb-0.5">Completed Claims</div>
            {/if}
            {#each data.claims as claim}
              {@const b = badge(claim.status)}
              <div class="bg-white rounded-xl border border-[#EAEAEA] px-4 py-4" style="box-shadow:0 1px 6px rgba(0,0,0,0.04);">
                <!-- Top row -->
                ...
                {#if claim.status === 'rejected' && claim.rejection_reason && expandedId === claim.id}
                  <div class="mt-3 rounded-lg p-3 border border-[#E53E3E] bg-[#fde8e8]">
